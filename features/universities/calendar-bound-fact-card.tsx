 ...
 */
export function CalendarBoundFactList({ title, items }: { title: string; items: CalendarBoundFactDisplay[] }) {
  if (items.length === 0) return null;
  return (
    <div className="space-y-2">
      <h3 className="text-sm font-medium text-muted-foreground">{title}</h3>
      <p className="text-xs text-muted-foreground">Evidence about competitiveness from a past cycle — not a requirement, and not something to aim for exactly.</p>
      <ul className="divide-y rounded-lg border">
        {items.map((fact) => (
          <li key={fact.id} className="space-y-1.5 px-4 py-2.5 text-sm">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <StatusBadge label="Past cycle" tone="neutral" icon={CalendarClock} />
            </div>
            <p>{fact.factText}</p>
            <p className="text-xs text-muted-foreground">{fact.nextCheckLabel}</p>
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
              {fact.retrievedAt ? <span>Checked {formatDistanceToNow(new Date(fact.retrievedAt), { addSuffix: true })}</span> : null}
              {fact.sourceUrl ? (
