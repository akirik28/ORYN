...
export function ResearchIdeaGenerator() {
  const t = useTranslations("profile.researchIdeaGenerator");
  const tIdeas = useTranslations("profile.researchIdeas");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={<Button variant="outline" size="sm" />}>
        <Sparkles className="size-4 text-primary" /> {t("trigger")}
      </DialogTrigger>
        ...
        <div className="space-y-4">
          <div className="flex items-end gap-2">
            <div className="flex-1 space-y-1.5">
              ...
              <Input id="research-field" value={field} onChange={(e) => setField(e.target.value)} placeholder={tIdeas("fieldPlaceholder")} />
            </div>
                ...
                <div key={project.researchQuestion} className="space-y-2 rounded-lg border p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <Badge variant="outline" className="shrink-0 capitalize">
                      {tIdeas(`difficulty.${project.difficulty}`)}
                    </Badge>
                  </div>
                  <p className="text-sm text-muted-foreground">{project.whyItFits}</p>
                  <div className="grid gap-1 text-xs text-muted-foreground sm:grid-cols-2">
                    <p>
                      <span className="font-medium text-foreground">{t("duration")}</span> {project.estimatedDuration}
                    </p>
                    <p>
                      <span className="font-medium text-foreground">{t("output")}</span> {project.expectedOutput}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs font-medium">{t("firstSteps")}</p>
                  ...
                  >
                    ...
                    {savedIndexes.has(index) ? t("added") : tIdeas("addToResearch")}
                  </Button>
                </div>
              ))}
            </div>
