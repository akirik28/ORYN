 ...
 *
 * SPLIT 2026-09-04 (B3a): the four sections below (Gap/Opportunities/Universities/
 * Applications) are now individually exported, each usable on its own dedicated route
 * (app/parent/(dashboard)/{progress,opportunities,universities,applications}/page.tsx) as
 * well as stacked here for the overview. `ParentPageShell` is the one background+container
 * wrapper both this file and every new page reuse, so there is exactly one place that owns
 * the gradient/max-w-3xl shape rather than five copies of it.
 */
export async function ParentPanelView({ data, locale = DEFAULT_LOCALE }: { data: ParentPanelData; locale?: Locale }) {
  ...
  return (
    <ParentPageShell>
      ...
      <OpportunitiesSection opportunities={data.opportunities} locale={locale} />
      <UniversitiesSection universities={data.universities} locale={locale} />
      <ApplicationsSection applications={data.applications} locale={locale} />
    </ParentPageShell>
  );
}

/**
 * The one background+container shape every page under app/parent/(dashboard)/ renders
 * inside -- extracted so the overview and the four dedicated section pages can't drift on
 * it the way two independent cookie-option literals already did once tonight. Not a layout
 * (App Router layouts can't easily take page-specific props like a page header can), just a
 * plain wrapper component each page calls directly.
 */
export function ParentPageShell({ children }: { children: React.ReactNode }) {
  return (
    ...
    >
      <div className="mx-auto max-w-3xl space-y-8 px-6 py-12">{children}</div>
    </div>
  );
}

/**
 * A page-specific title block, for the four dedicated section pages (the overview keeps its
 * own richer header above, inline in ParentPanelView -- this is deliberately plainer, one
 * line of context under the heading rather than the overview's three).
 */
export function ParentSectionHeader({ title, description }: { title: string; description: string }) {
  return (
    <div className="space-y-1">
...
export function OpportunitiesSection({ opportunities, locale }: { opportunities: ParentPanelData["opportunities"]; locale: Locale }) {
  const tr = locale === "tr";
  return (
    <Section
      icon={Compass}
...
export function UniversitiesSection({ universities, locale }: { universities: ParentPanelData["universities"]; locale: Locale }) {
  const tr = locale === "tr";
  return (
    <Section
      icon={GraduationCap}
...
export async function ApplicationsSection({ applications, locale }: { applications: ParentPanelData["applications"]; locale: Locale }) {
  ...
  // `locale` prop every other string in this tree already respects -- confirmed live
 ...
 */
export function GapSection({ gap, locale }: { gap: ParentPanelData["gap"]; locale: Locale }) {
  const tr = locale === "tr";
  const body = (() => {
