 ...
 * an honest checkout CTA (real once a payment provider exists, never faked before then),
 * and no invented capabilities in the comparison table. Same mocked-Server-Action-boundary
 * approach as entity-combobox.test.tsx.
 *
 * 2026-09-04: the original "no buy button" constraint was superseded the same night by the
 * founder's own direct instruction to add real payment ("ödeme sistemini bugün ekliycem") —
 * see PlanTierView's own header for the full history. What survives, and what this file now
 * tests instead: `startUltraCheckoutAction` is the single source of truth for whether
 * checkout is actually available, and the component must render exactly what it reports —
 * never a fabricated success, never a dead button with no explanation.
 */

vi.mock("@/app/(app)/upgrade-interstitial-actions", () => ({
  startUltraCheckoutAction: vi.fn(),
}));
...
const mockedStartCheckout = vi.mocked(startUltraCheckoutAction);
...
beforeEach(() => {
  ...
  // The real, current, honest state as of this pass: no payment provider is configured yet
...
describe("PlanTierView — honest checkout CTA", () => {
  test("with no payment provider configured, the CTA is disabled and says so plainly — never a dead button with no explanation", async () => {
    renderView("standard");
    expect(screen.getByText("Get more out of Proxola")).toBeInTheDocument();
    // The live price, formatted — 05's formatPrice, not a hardcoded string. Confirms the
    // component actually rendered the prop-driven price, not a stale hardcoded copy.
    expect(await screen.findByText(/399\.99 TL\/month/)).toBeInTheDocument();

    const button = await screen.findByRole("button", { name: "Upgrade to Ultra" });
    expect(button).toBeDisabled();
    expect(screen.getByText(/Payment isn't set up yet/)).toBeInTheDocument();
  });

  test("clicking the disabled CTA never calls window.location or shows a fake success state", async () => {
    renderView("standard");
    const button = await screen.findByRole("button", { name: "Upgrade to Ultra" });
    fireEvent.click(button);
    // A disabled button's onClick never fires in the DOM at all — this is the actual
    // guarantee, not just an absence of visible success copy.
    expect(screen.queryByText(/success|confirmed|complete/i)).not.toBeInTheDocument();
  });

  test("once a provider is configured, the CTA becomes a real, enabled control", async () => {
    mockedStartCheckout.mockResolvedValue({ status: "ready", checkoutUrl: "https://checkout.example.com/session/abc" });
    renderView("standard");
    // Re-queries fresh each poll rather than holding a reference captured before the async
    // result resolves — the "not configured" and "ready" branches render structurally
    // different trees (a disabled button wrapped with explanatory text vs. a bare enabled
    // one), so React mounts a new element rather than toggling one node's disabled attribute.
    await waitFor(() => expect(screen.getByRole("button", { name: "Upgrade to Ultra" })).not.toBeDisabled());
    expect(screen.queryByText(/Payment isn't set up yet/)).not.toBeInTheDocument();
  });

  test("a provider-reported error is shown verbatim, not swallowed into the generic not-configured copy", async () => {
    mockedStartCheckout.mockResolvedValue({ status: "error", message: "The payment provider is temporarily unreachable." });
    renderView("standard");
    expect(await screen.findByText("The payment provider is temporarily unreachable.")).toBeInTheDocument();
    expect(screen.queryByText(/Payment isn't set up yet/)).not.toBeInTheDocument();
  });

  test("an ultra-tier student sees no checkout card at all — they have nothing to upgrade to", () => {
    renderView("ultra");
    expect(screen.queryByText("Get more out of Proxola")).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Upgrade to Ultra" })).not.toBeInTheDocument();
