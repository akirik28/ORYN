...
describe("migration numbering", () => {
  test("0058 is not duplicated, even though a later migration now exists", () => {
    ...
    // directory forever; migration 0061 (public_profiles_require_authenticated,
    // 2026-08-22 — the RLS verification package's own fix, unapplied like 0058) is the
    // one that currently follows it. Bump the literal below when the next migration
    // lands, the same way this one did — it is a collision guard, not a permanent
    // ceiling.
    ...
    expect(Math.max(...numbers.map(Number))).toBe(61);
  });
});
