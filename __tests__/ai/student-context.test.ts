...
describe("formatContextForPrompt — assessed dimensions are pre-sorted weakest to strongest", () => {
  test("reproduces oryn-80's exact fixture shape: the two real weakest appear first, in the correct order", () => {
    ...
    // Third pass (docs/advisor-rank2-narrowing-instruction-2026-09-03.md): tagging rank 2 the
    // same way as rank 1 reached the model correctly but didn't reliably close the symptom
    // live (4/5 twice, independently) -- the tag is gone now, replaced by an instruction not
    // to make the claim at all. Neither Entrepreneurship nor Career Exploration should carry
    // any ordinal tag; the correct top-to-bottom order is still the only signal given.
    expect(lines.find((l) => l.includes("Entrepreneurship"))).not.toMatch(/weakest/);
    expect(lines.find((l) => l.includes("Career Exploration"))).not.toMatch(/weakest/);
  });

  test("the sole minimum gets an explicit weakest tag; the only other dimension gets no ordinal tag at all", () => {
    const context = baseContext({
      profileScores: [
        { dimension: "academics", score: 85, confidence: "high", state: "strong" },
        { dimension: "awards_distinction", score: 20, confidence: "medium", state: "emerging" },
    ...
    // Third pass: no second-weakest tag exists any more, for any dimension, regardless of how
    // few assessed dimensions there are -- the header instruction now tells the model outright
    // not to make this claim, rather than the data quietly implying one.
    const academicsLine = text.split("\n").find((l) => l.includes("Academics"))!;
    expect(academicsLine).not.toMatch(/weakest/);
  });
  ...
  test("the header withholds ordinal claims past the tagged weakest, rather than asking for a correct one", () => {
    const text = formatContextForPrompt(baseContext());
    expect(text).toContain('Only the dimension(s) explicitly tagged "weakest" may be named by rank');
    expect(text).toContain("do not claim which dimension is second-weakest, third-weakest, or any other ordinal position");
  });
});
