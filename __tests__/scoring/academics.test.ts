  ...
  describe("course-grade-presence fallback (no overall_gpa on file)", () => {
    ...
    test("does not engage the fallback, and does not double-count, when overall_gpa is already present", () => {
      const withGpaOnly = scoreAcademics(
        facts({
          educationRecords: [
            { id: "e1", user_id: "u1", school_name: "S", school_entity_id: null, country_entity_id: null, country: "US", stage: "high_school", curriculum: "ap", curriculum_other_text: null, start_date: null, end_date: null, is_current: true, overall_gpa: 3.5, gpa_scale: 4.0, notes: null, created_at: "", updated_at: "" },
          ],
        })
      );
