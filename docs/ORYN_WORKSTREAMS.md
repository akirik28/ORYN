...
| DE-NL-PROGRAMMES | Claude (same overnight session, pivoted to this lane 2026-08-21 ~17:30+ local) | `oryn/de-nl-programmes-research` (isolated worktree at `.claude/worktrees/de-nl-programmes`, branched from `origin/main`@`5b01d7d`) | Programme-catalogue deepening for 5 named universities, assigned by the coordination session. A live query taken before dispatch showed the real starting state: LMU 4, Heidelberg 0, TU Berlin 0, Bonn 0, Delft 15-Bachelor's-only. **890 verified records across 5 files** (`de_nl_batch1-5_2026-08-21.jsonl`) — LMU 180 (61B+119M), Heidelberg 271 (136B+135M), TU Berlin 142 (50B+92M, matches the university's own stated totals exactly), Bonn 257 (131B+126M), Delft 40 (2 Bachelor's gap-fill + 38 Master's). Language-of-instruction discipline held at real cost: Bonn alone has 87/257 null because the source itself omits the field, not guessed from programme name. One credential-handling judgment call worth noting: LMU's finder needed a Basic-auth endpoint the researcher didn't have credentials for — it observed the page's own authenticated requests instead of acquiring or bypassing the credential. **Live-data discrepancy found and reported, not silently absorbed**: the task brief described RWTH Aachen and FU Berlin as "already covered" (grouped with KIT) and VU Amsterdam as "10 of 29" — a live query (repeated twice) shows all three at 0; only KIT (45) is accurate. None of the three were in this lane's assigned scope. Full detail: `docs/research/university-programs-de-nl/README.md`. | `data/research/university-programs/de_nl_*.jsonl`, `docs/research/university-programs-de-nl/**`. Research/evidence only, no schema/app/production/Supabase writes. | **Complete for this pass.** All 5 batches + summary doc individually committed and pushed. Zero duplicate `research_program_id`/`official_program_url` within or across files, validated locally before commit. |
