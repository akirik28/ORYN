...
| RESEARCH | Research Claude | `oryn/research-turkey-schools` (isolated worktree at `.claude/worktrees/research-intelligence`, branched from `origin/main`@`5c59115`; branch name is now Wave-1-specific but the lane also carries R2.1 below — not renamed mid-package to avoid branch churn) | Two packages complete this checkpoint: (1) Turkey high-schools canonical registry, Wave 1 — 100/100 (58 already-canonical + 42 new candidates, incl. resolving all 8 previously-unresolved `entity_verification_queue` names). (2) Secondary-education-system intelligence (R2.1) — 8/8 systems (MEB, IB DP, AP, Cambridge IGCSE, Cambridge AS/A-Level, French Bac, German Abitur/DSD, US diploma), incl. a confirmed real schema-modeling error (`education_records.curriculum='ap'`) and a predicted-vs-final-grade gap, flagged for Claude B. | `docs/research/**`, `data/research/schools/**`, `data/research/academic-systems/**`, `docs/handoffs/research-turkey-high-schools.md`, `docs/handoffs/research-secondary-education-systems.md` — research/evidence only, no schema/app/production writes | Both packages complete and pushed. Handoffs: `docs/handoffs/research-turkey-high-schools.md` (consumer: DATA-A / canonical-entity ingestion) and `docs/handoffs/research-secondary-education-systems.md` (consumer: PROD-B / counselor academic-profile semantics). This lane does not write to Supabase or production code — both outputs are reviewed proposals. Next: re-checking `docs/current-state.md`/this file for the highest-leverage next research package (candidates: country-level admissions-system intelligence as a natural R2.1 sequel, or opportunity-eligibility research — `opportunities.eligible_countries` measured live this checkpoint at 14/334 rows populated, still the single largest data-trust gap blocking real eligibility matching per `docs/MASTER-EXECUTION-STRATEGY.md` §6 P3). |
