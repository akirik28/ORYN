...
| RESEARCH | Research Claude | `oryn/research-turkey-schools` (isolated worktree at `.claude/worktrees/research-intelligence`, branched from `origin/main`@`5c59115`; branch name is now Wave-1-specific but the lane carries R2.1/R3.1/R4 too — not renamed mid-package to avoid branch churn) | Four packages complete this checkpoint: (1) Turkey high-schools canonical registry, Wave 1 — 100/100. (2) Secondary-education-system intelligence (R2.1) — 8/8 systems, incl. a confirmed schema-modeling error (`education_records.curriculum='ap'`) flagged for Claude B. (3) Country-level admissions-system intelligence (R3.1) — 6/6 destination systems (US/UK/Netherlands/Italy/Germany/Canada), each with a dedicated Türkiye-applicant section and a 12-rule evidence-based ruleset; key finding: DSD-vs-academic-qualification-recognition conflation (Germany) is the highest-stakes counselor risk found across all 3 packages. (4) Opportunity eligibility intelligence (R4, self-directed — ranked #1 of 3 candidates in the R3.1 wrap-up) — grade/age normalization across 6 systems (verified UK Year *N* = US Grade *N-1* off-by-one via an official UK gov source; US↔Türkiye is the one safe same-number pair), a citizenship/residency/school-location 3-axis taxonomy across 20 real programs, prerequisite/timing patterns across 5 opportunity categories, plus a 32-row sourced extraction-audit sample against 102 live `opportunities` rows and a 14-rule evidence-based ruleset. | `docs/research/**`, `data/research/schools/**`, `data/research/academic-systems/**`, `data/research/admissions-systems/**`, `data/research/opportunity-eligibility/**`, `data/research/opportunities/**`, `docs/handoffs/research-*.md` — research/evidence only, no schema/app/production writes | Five packages complete and pushed. Handoffs: `docs/handoffs/research-turkey-high-schools.md` (consumer: DATA-A), `docs/handoffs/research-secondary-education-systems.md` and `docs/handoffs/research-admissions-systems.md` (consumer: PROD-B), `docs/handoffs/research-opportunity-eligibility.md` (consumer: DATA-A for the extraction-audit sample, PROD-B for the ruleset), `docs/handoffs/research-opportunities-counseling-list.md` (5th package, consumer: DATA-A — closes the founder's "Section 12" counseling-materials discovery-list gap in `docs/research-handoff-opportunities.md`: 2 new verified candidates, 4 selectivity-evidence update proposals, and 2 unplanned data-quality findings — a university-only "Hack-AI-thon" row wrongly live as a high-school opportunity, and a likely Wharton Data Science Competition / Sports Analytics duplicate pair). This lane does not write to Supabase or production code — every output is a reviewed proposal. Live-remeasured this checkpoint: `opportunities.eligible_countries` moved from 14/334 (R2.1 checkpoint) to only 18/352 (~5.1%) — the gap has not materially closed. |
