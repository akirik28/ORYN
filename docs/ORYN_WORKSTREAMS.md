...
|---|---|---|---|---|---|
| DATA-A | Claude A | `oryn/programs-pipeline-reconciled` | Admissions-URL coverage campaign (4 batches, 512/1019 universities, 50.2%) + programme-catalogue pipeline (Delft/Trinity pilot + Edinburgh/Waterloo/Glasgow batch 2, 6 real extraction bugs found and fixed with tests) + opportunities Wave 1 groups B/C/D/F/G applied + dedup QA cleanups + **0043 data backfill run and verified (9/9 pairs correctly `duplicate_status='superseded'`)**. | University/opportunity data acquisition, canonical graph, ingestion pipelines, data QA — see `docs/MASTER-EXECUTION-STRATEGY.md` §3 "Computer A" for the full scope list | **Merged to `main` 2026-08-21** at tip `7f27734`, after the branch reached a stable session-close. Integration Manager ran the full gate on that exact tip (lint, typecheck, 1155/1155 tests, build 39 routes, zero new migrations — 0049+ confirmed free) and classified the diff as 11 files / 1319 lines scoped to the acquisition engine with no overlap into counselor/scoring/eligibility/canonicalization. The only conflict was this file, resolved in favour of `main`'s newer rows. |
...
| ADMISSIONS-INTEL | Claude (overnight research session, 2026-08-21 ~01:2x–11:00 local; one bounded follow-up commit after 11:00 under explicit founder authorization via the coordination session) | `oryn/admissions-intelligence-research` (isolated worktree at `.claude/worktrees/admissions-intelligence`, branched from `main`@`5ec6700`) | R3.1 continuation/expansion: the existing 6-country package (US/UK/Netherlands/Italy/Germany/Canada) stays a closed, unmodified deliverable. Added 8 more countries (Switzerland, France, Spain, Ireland, Australia, New Zealand, Hong Kong, Singapore) — same template/schema/source-priority standard, same dedicated Türkiye-applicant section per country; 14 countries total in `admissions-systems-v1.json` (internal version marker `admissions-systems-v2`). **Fully integrated**: README.md's cross-country matrix and countries-covered table now cover all 14 (original 6 columns preserved byte-for-byte, 8 appended), and RULE-ADMISSIONS-013 through 016 added (001-012 unmodified) — 013/014 kept deliberately separate (eligibility-gate vs. evidence-model-switch), 015 = Singapore nationality-quota capacity axis, 016 = cross-country terminology false-cognate risk. Batch 3 (Sweden/Belgium/Austria/Poland/Czech Republic) fully scoped, explicitly NOT started per founder instruction to pause further project advancement. Full detail: `docs/handoffs/research-admissions-systems-v2.md`. | `docs/research/admissions-systems/*.md` + `README.md` (existing 6 countries' matrix cells untouched, additive only), `data/research/admissions-systems/**`, `docs/handoffs/research-admissions-systems-v2.md`. Research/evidence only, no schema/app/production writes — same posture as the RESEARCH row. | **Complete for this session's authorized scope.** Every committed file is finished/validated, README integration done, no half-written docs. Genuinely idle now — no pending wakeup, no batch 3, awaiting the founder's post-reorg reassignment (per his direct instruction to route through the coordination session). Zero file-path overlap with COUNSEL-RESEARCH or any code/data lane. |

| OPPS-NIGHT | Claude (overnight research session, started 2026-08-21 01:2x local) | `oryn/night-opportunities-research-2026-08-21` (worktree `.claude/worktrees/night-opportunities-research`, branched off `origin/oryn/programs-pipeline-reconciled`@`60d52a3`) | Global opportunities-intelligence research campaign per founder's "ORYN NIGHT RESEARCH" brief: fill category gaps (hackathon/conference/student_program at zero; volunteering/fellowship/entrepreneurship/academic_program/online_program/internship/scholarship/research all far behind summer_program's 252) and geography gaps (Middle East/East Asia/South Asia/Australia-NZ at zero; Canada/Turkey/continental Europe thin). See `data/research/opportunities/NIGHT_RESEARCH_2026-08-21.md` for full baseline + wave log. | `data/research/opportunities/*.jsonl` (new files only, own naming prefix `night1_*`/`night2_*`), `data/research/opportunities/NIGHT_RESEARCH_2026-08-21.md`. **Explicitly does not**: write to production DB (no `execute_sql` inserts/updates), run the ingest scripts, touch schema/migrations, or edit application code — research/evidence output only, for a later separate ingestion pass. | **Ended at deadline (2026-08-21 11:00 Europe/Istanbul), Wave 1 mostly complete.** 51 new page-verified records committed+pushed (HEAD `552dd2b`, matches origin exactly) across 4 batches: Middle East+South Asia (16), East Asia+Australia/NZ (11), Canada+Turkey+continental Europe (19), self-research closing the 3 zero-coverage categories (5). One Wave-1 slice (international olympiads) never landed — its agent ran 9h without completing/replying to a finalize-now request; **`night1_2026-08-21_groupOLYMPIADS.jsonl` should be checked for partial output before re-dispatching.** Wave 2 (non-US internships/fellowships/scholarships, hackathons, arts/business competitions) never dispatched — no time remaining. Full final report with category/geography distribution, needs-review leads, and exact ingestion handoff in `data/research/opportunities/NIGHT_RESEARCH_2026-08-21.md`. Cross-checked against `oryn/programs-pipeline-reconciled`'s own concurrent `wave4`/`wave5` opportunity research (10 records) per a coordination-session flag — zero real duplicates. Not merged into `oryn/programs-pipeline-reconciled` — left on its own branch for the founder/DATA-A to integrate. |
...
- **Migration 0047 had a real live-crash bug, now fixed**: `lib/opportunities/
  ...
  `oryn-qa-scratch`.
- **RESOLVED 2026-08-21**: the Supabase-secret-key problem was not a JWT-clock regression in
  the end — `.env.local`'s `SUPABASE_SECRET_KEY` was simply empty (as were `ANTHROPIC_API_KEY`,
  `TAVILY_API_KEY`, `CRON_SECRET`). The founder supplied a current `sb_secret_…` key and
  `npm run check:integrations` now reports Supabase **and** Supabase (secret key) both OK for
  the first time. This unblocked the ingestion write path; `ANTHROPIC_API_KEY` and
  `TAVILY_API_KEY` remain empty, so every AI-backed feature and Tavily discovery is still dead.
- Superseded note, kept for the record: a new Supabase-secret-key regression ("JWT issued in future"),
