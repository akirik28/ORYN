...
| `empty-field-measurement-2026-09-04.md` | Live null/empty-field counts across `opportunities`/`universities`, explicit "measurement only, nothing written" | Live DB read, 2026-09-04, no commit cited (a data snapshot, not a code claim) | Not checked in this pass — this is the one row where "what merged after" is the wrong question; the right one is "has anything written to these tables since," which is a live DB re-query, not a git log | **Not assessed — different kind of claim, and about to expire on its own terms regardless.** This is a measurement of *data*, not of code behavior, so its expiry condition is a write to `opportunities`/`universities`, not a merge — `git log` cannot answer whether it's stale. Concretely, though: every number in that doc is now also older than the founder's own migration run, which lands this morning. Whatever those counts are today, they describe a database that's about to change on purpose — worth treating as a pre-migration snapshot, not a current baseline, the moment that run happens. |
...
## Update — same night, before the ink dried

Three of the five rows above that weren't already-resolved-or-acknowledged closed out
within about an hour of this audit being pushed. Not for the same reason in each case,
worth being precise about which:

- **`veli-hesabi-spec`'s lane table — caused by this audit.** `da1f15f1`'s own commit
  message names it directly: "The expiry audit flagged it... section 8's table,
  timestamped 01:40, still showed P2 and P3 in progress and P5 not started, all three
  long since landed."
- **The two parent-bug rows (`parent-state-machine-trace`, `parent-surfaces-mobile-
  pass`) — not caused by this audit.** Both were already named "in flight" with 71 at
  the time this audit ran; `063e716f` (merged `60048cfa`) landed them together with a
  third, unrelated parent bug per the CEO's own bundling call. This audit correctly
  recorded them as "still valid, not yet fixed" at the moment it checked — that moment
  just didn't last long.

Each row got its own inline update above rather than being rewritten, so the table still
shows what was true when this audit ran, not only what's true now — same discipline as
every other row here. What's left genuinely open: `empty-field-measurement`, which by its
own nature (a data snapshot, not a code claim) isn't the kind of thing a commit closes —
only a live re-query can, and per the note above it's now also aging toward the founder's
migration run.

Worth naming plainly: a verification-expiry audit going three-rows-stale within an hour
of being written is exactly the phenomenon this doc is about. Not a flaw in the audit —
the same night-across-seven-lanes velocity that motivated it in the first place — but a
clean demonstration that "verified" is always a timestamp, including this one.
