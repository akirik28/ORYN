...
- ~~**Application readiness (Phase 70)** — there is no application readiness anywhere.~~
  **WRONG, corrected 2026-09-01 22:40 — it existed, and it was worse than missing.**
  `lib/applications/readiness.ts` was already wired into both `/applications` pages with tests.
  I ran `grep -rn "readiness" ... | head -8`, saw only `lib/opportunities/readiness.ts` and
  `profileReadiness`, and concluded from the visible part of a truncated search — **the same
  can't-tell-nothing-found-from-didn't-look failure this document's closing section is about.**

  What it actually did was show a bare percentage with two dishonest cases: an empty checklist
  returned **0%** rather than "unknown" (the original test even asserted "0%, not NaN" — solving
  the crash without asking whether zero was true), and a **submitted** application still showed a
  completion figure, so the live row with 1 of 8 requirements ticked rendered **"13% ready" beside
  a "Submitted" badge**. Now fixed: `computeReadiness` returns a discriminated union —
  `unmeasured` / `not_tracked` / `measured` — so a caller cannot render a percentage without
  first branching on what it is.
