...
## FYI: 8 duplicate `universities` rows merged at the identity layer (canonical_entities), not yet at the `universities` row layer
...
(Australia), and Al-Farabi Kazakh National University each had two `universities` rows
...
Warwick", etc.). `canonical_entities`/`entity_aliases`/`entity_external_ids` are merged now
(`merge_canonical_entities()`), but the two `universities` rows themselves both still exist —
I deliberately did not delete or touch either, specifically because 4 of these 8 pairs
...
action needed. If you're about to add programs/requirements for any of these 8 institutions
