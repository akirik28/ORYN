...
### Staged cleanup — NOT applied, hardened for a founder to actually run

`data/research/opportunities/description_contamination_cleanup_2026-09-02.sql`:
- 35 `UPDATE` statements (of the 37 contaminated rows; 2 more below are dedup flags, not rewrites),
  each keeping only verifiable factual claims (dates, costs, requirements, program structure) from
  the row's current text and dropping every sentence that is process narration, tool-call
  self-reference, sourcing-confidence hedging, or cross-references to the session's own prior work.
...
- **Self-verifying, per CEO's request.** Every WHERE clause now also checks `left(description, N)
  = '<exact prefix as read on 2026-09-02>'` — if another lane touched a row before the founder
  runs this, that statement returns `UPDATE 0` (visible, safe) instead of silently overwriting
  their edit. All 35 guard literals were generated by Postgres itself (`quote_literal`), not
  hand-typed, and dry-run verified as a read-only SELECT before finalizing — which caught two real
  bugs: one guard was missing a trailing space, and Ringling's source text uses a curly apostrophe
  (’, U+2019) where I'd written a straight one. Both fixed and re-verified. The full 35-statement
  batch was then run as `EXPLAIN` (parses/plans, never writes) with zero errors, confirming valid
  syntax and correct quote-escaping throughout. The file's header states plainly what a correct
  run looks like (`UPDATE 1` × 35) and what to do if any statement returns `UPDATE 0`.
- **Not applied to the live DB.** Every write to this data is founder-gated per standing rule.
