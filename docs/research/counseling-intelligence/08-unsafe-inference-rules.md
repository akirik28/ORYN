...
| Treating every Casper-marketed field the same way regardless of confirmed adopter's degree level | RULE-COUNSEL-244/245 (this session) — Western University's *undergraduate* engineering program is a confirmed, ORYN-population-relevant adopter; Dalhousie's Casper-using business program is an *MBA* (graduate), a decade-plus past ORYN's population's actual admissions stage — same assessment category, materially different relevance | `06-major-family-evidence/05-engineering-me-ce-ae-ee.md`, `08-business-management-entrepreneurship.md` |
