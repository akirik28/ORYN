...
| IB/A-Level/AP subject selection | The actual decision point for A-Level (chosen entering Year 12); for IB, **the IB organization itself does not standardize a single timeline** — ibo.org's own official Diploma Programme pages leave subject-selection timing to individual schools rather than mandating one point in the two-year DP [S-IB-SUBJECTS], so "Grade 10/11 transition" should be read as a common school-level convention, not an IBO-mandated date | Already committed at whatever point the student's specific school set — recommendations should work within the chosen subjects, not suggest reopening the choice |
...
**A refinement found late in this session, worth stating precisely rather than leaving the exam-
dominance claim at "activities play a minimal role":** YKS placement score is not pure exam score
either — it also folds in OBP (Ortaöğretim Başarı Puanı, a school-achievement score derived from
the student's diploma grade), applied at a small, ÖSYM-published coefficient (commonly described as
~0.12 of the placement-score formula, roughly a maximum of 60 points on a scale that runs into the
hundreds, and halved to ~0.06 for a candidate who has been placed in a program once before)
[S-YKS-OBP]. **RULE-COUNSEL-242:** this does not weaken RULE-COUNSEL-228's core claim — it
sharpens it. The one channel besides exam-day performance that reaches a YKS placement score is
still purely grades-based (this package's `academics` dimension, via the diploma-grade input to
OBP), capped small, and structurally distinct from anything an extracurricular activity, award, or
leadership role could contribute. There remains no channel at all by which `leadership`,
`community_impact`, `entrepreneurship`, `research`, or `awards_distinction` evidence reaches YKS
placement — OBP does not change that. **Sourcing honesty on this specific figure:** the ~0.12/~0.06
coefficient and the halving-on-reuse rule are corroborated by several independent Turkish
student-facing YKS-calculator sources describing ÖSYM's published methodology consistently with
each other, not by this session directly re-deriving the number from ÖSYM's own current-year
coefficient table — a direct fetch of ÖSYM's own site (`osym.gov.tr`) this pass confirmed that ÖSYM
does publish exact numeric coefficient tables for its placement-score methodology as standing
institutional practice (e.g., a historical "ağırlıklı ortaöğretim başarı puanının çarpılacağı
katsayılar" table hosted directly on its own domain), which corroborates that a precise, official,
non-improvised coefficient governs this and is consistent with the commonly-quoted ~0.12 figure —
but the current year's exact table was not independently re-opened and read this pass. Treat the
existence and rough size of the OBP channel as high confidence, the exact current-year coefficient
value as medium.
...
| S-IB-SUBJECTS | [ibo.org — DP curriculum](https://ibo.org/programmes/diploma-programme/curriculum/), [Example diploma subject choices](https://ibo.org/programmes/diploma-programme/curriculum/example-subject-choices/), [Course selection guidance](https://ibo.org/university-admission/support-students-transition-to-higher-education/course-selection-guidance/) — official IBO pages, located directly at ibo.org (a genuine upgrade from this document's earlier "no single official page found" note); full page content could not be fetched directly (ibo.org blocks direct automated fetching), so this entry is grounded in confirming these official pages exist and in the WebSearch-returned summary of their content, not a full-page read | Official (International Baccalaureate Organization) for the pages' existence and general content; the specific "no IBO-mandated timeline, school-determined" finding is medium-high confidence (consistent with what IBO's own indexed page summaries show, not independently confirmed via full-page fetch) | §3 IB subject-selection row, §1 IB staging |
...
| S-YKS | [Student Selection and Placement System — overview](https://en.wikipedia.org/wiki/Student_Selection_and_Placement_System) corroborated by multiple independent sources describing ÖSYM's official YKS structure (TYT/AYT/YDT sessions, score-ranked preference placement); ÖSYM's own site (osym.gov.tr) directly fetched this pass ([YKS exam-group index](https://www.osym.gov.tr/SinavGrubu/Index/2), [programs/quotas guide index](https://www.osym.gov.tr/yuksekogretim-programlari-ve-kontejanlari-kilavuzu)) to confirm structure, though the specific current-year coefficient tables themselves were not opened (see S-YKS-OBP) | Secondary description of an official, government-run exam system, now partially corroborated by a direct fetch of the official domain itself | Medium-high — the system's existence and general structure (centralized exam, score-based placement, no holistic application file) is well-corroborated, low-controversy, and now partially confirmed directly against osym.gov.tr; exact current-year numeric coefficients still rest on secondary sourcing (see S-YKS-OBP) | §6 |
| S-YKS-OBP | Multiple independent Turkish student-facing YKS score-calculator sources (e.g. multibem.com.tr, gridanismanlik.com, egitimtime.com, unikazan.com) converging on the same OBP (Ortaöğretim Başarı Puanı) formula and ~0.12/~0.06 coefficients, describing ÖSYM's published methodology; a direct fetch of `osym.gov.tr`'s programs/quotas guide page this pass confirmed ÖSYM hosts precise numeric coefficient tables for placement-score calculation as standing practice (e.g. a historical "ağırlıklı ortaöğretim başarı puanının çarpılacağı katsayılar" table on its own domain) without this pass opening the current-year table itself | Secondary (convergent, multi-source) for the specific coefficient values; official (osym.gov.tr, directly fetched) for confirming that ÖSYM publishes exact coefficient tables as a matter of practice | High for the existence and rough size of the OBP channel (diploma-grade-based, small, capped); medium for the exact current-year ~0.12/~0.06 coefficient values (not independently re-derived from ÖSYM's own current table this pass) | §6 |
