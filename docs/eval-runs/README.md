...
| `2026-09-02-run5-sonnet-production-path.log` | claude-sonnet-5 | **317/360. The first run that measures the PRODUCT, not the model** — harness now applies `resolvePlanSelfContradiction` + `enforceTimeBudget` exactly as production does, fixtures corrected to real enum shapes (`5_10h` not `"5-10 hours"`, `ib` not `IB`), six raw-enum leaks closed, age framing added to the prompt. **Not comparable to the 325/360 baseline** — that measured raw model output. $0.3112. **0 deterministic findings, 0 post-processing interventions.** |
