...
export function evaluateRequirement(
  ...
  if (qualifiers.evaluationGate) return gateResult(qualifiers.evaluationGate);
...
export function evaluateRequirementGroup(members: RequirementGroupMember[], facts: RequirementFacts): RequirementGroupEvaluationResult {
  ...
  for (const member of members) {
    memberResults.set(member.id, evaluateRequirement(member.category, member.rawStructuredRule, facts, member.rawQualifiers));
  }
