 ...
 * is legitimate at all*, as opposed to what the comparison is. The columns migration 0056
 * proposes on `university_requirements` (`evaluation_gate`, `test_scale`, `scale_ambiguity`,
 * `recency_rule`, `excluded_provenances`) plus migration 0052's `is_exclusion`, camelCased
 * the same way `RequirementFacts` camelCases its own source columns — the caller maps
 * snake_case DB columns onto this shape.
 *
 * `is_exclusion` belongs here for exactly the reason the 0056 columns do: it is a property
 * of the requirement that decides whether the threshold in `structured_rule` may be
 * compared at all, not a property of the comparison. It is also the one field of this
 * interface that exists in production today (0052 IS applied), which is why the whole row
 * is what callers pass — one argument carries both the applied column and the pending ones.
 *
 * 0056 is written and NOT applied, so every field here except `isExclusion` is absent in
 * production today. That is deliberately safe in one direction only: an absent qualifier
 * can never *unlock* a comparison the evaluator would otherwise refuse, it can only fail to
 * block one. The two qualifiers that must block on their own (an unqualified TOEFL/TR-YÖS
 * threshold, a percentile threshold) are therefore detected from the rule itself as well —
 * see `evaluateTestScore` — rather than relying on a column that does not exist yet.
 */
export interface RequirementQualifiers {
  ...
  /**
   * Migration 0052's `university_requirements.is_exclusion`, whose column comment states the
   * contract this implements: "Never auto-resolved by evaluate.ts — always
   * needs_manual_review". An exclusion states who is NOT eligible, so reading its text as a
   * threshold to clear inverts it and tells an excluded student they qualify.
   *
   * Resolved to the `eligibility_restriction` gate rather than given a parallel code path,
   * because that gate already means precisely this. An explicit `evaluation_gate` on the
   * same row wins — it is the more specific statement of why the row is unevaluable — and
   * both answer needs_manual_review either way, so the precedence is about copy, not safety.
   */
...
export interface RequirementGroupMember {
   ...
   *
   * Kept as its own field rather than left to `rawQualifiers` because it does two jobs: it
   * gates the member's own verdict (via the `eligibility_restriction` gate, see
   * `RequirementQualifiers.isExclusion`) AND it forces the whole group to manual review.
   * The second is a group-level consequence no per-member qualifier has. */
  isExclusion?: boolean;
   ...
   *
   * Distinct from `isExclusion` above precisely because of that scope difference: a gate
   * confines itself to the alternative that carries it, an exclusion condemns the group. */
