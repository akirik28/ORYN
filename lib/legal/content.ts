...
export const LAWYER_FLAGS: LawyerFlag[] = [
  ...
  {
    id: "minorToMinorMessaging",
    question:
      "This product's own AGENTS.md section 12 states 'do not build public student messaging in V1.' A 1:1 messaging feature between students exists in the codebase, built after that line was written, under a founder-approved scope change. Is 'built, tested, and switched off by a kill switch, with a documented scope decision on file' an adequate answer to a regulator or a parent asking whether student-to-student messaging exists — or does having built it at all (regardless of whether it is currently reachable) itself require a review before it could ever be switched on?",
    currentState:
      "supabase/migrations/0023_social_v1.sql and 0027_messaging.sql: mutual-consent connections (request → accept, not an open follow) and 1:1 messaging, gated to an accepted connection only and re-checked server-side at send time (not just hidden by the UI), with blocking (blocked_users) and abuse reporting (message_reports) also built. app/(app)/messages/actions.ts's sendMessage() is a real, callable Server Action that inserts into messages — this is not unused schema. " +
      "This was not accidental drift: docs/product-decisions.md records a founder-approved scope change (2026-08-15, 'Chat 2 pass') that repositioned the product to add a Professional Network pillar, and the session that built it quotes this exact AGENTS.md line and chose mutual-consent connections specifically because an open-follow model 'lets any signed-in stranger attach themselves to a minor's profile with no consent step.' Additional mitigations built at the same time: no people-search/student directory (lib/search/ only ever searches the caller's own data), a Connect button only ever appears on an already-public profile, and messages are denormalized so history survives a later disconnect or block (for abuse-report integrity) rather than being erased. " +
      "Both messaging and connections are gated behind server-only env-var kill switches (ORYN_ENABLE_MESSAGING, ORYN_ENABLE_CONNECTIONS), exact-string-match 'true' only, deliberately absent from .env.example so a new developer cannot accidentally think to set them. Verified 2026-09-05 that the gate is actually invoked, not just defined, at every page and Server Action entry point (requireMessagingEnabled()/requireConnectionsEnabled() called at the top of every relevant page; assertMessagingEnabled()/assertConnectionsEnabled() called first in every relevant Server Action) — a dedicated test suite checks this per function, not per file, specifically because an earlier, similar flag's test only checked that the assertion string appeared somewhere in the file. " +
      "Not independently verified this pass: the actual live value of ORYN_ENABLE_MESSAGING/ORYN_ENABLE_CONNECTIONS in the real deployed hosting environment. The code default and three internal audit documents (most recently 2026-09-02) all state both are unset there, but nobody has queried the hosting provider's own configuration directly to confirm the current live value — flagged to the founder as the one claim in this file that is verifiable but has not yet been verified by anyone.",
  },
];
