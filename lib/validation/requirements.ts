...
/**
 * Reads migration 0056's qualifier columns off a `university_requirements` row. Input keys
 * are the snake_case column names; output is the camelCase `RequirementQualifiers` the
 * evaluator takes. Extra keys are stripped, so the whole row can be handed to it.
 *
