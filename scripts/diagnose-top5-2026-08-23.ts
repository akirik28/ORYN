#!/usr/bin/env node
/**
 * Read-only diagnostic for Objective 3 (top-5 requirements/deadlines dry-run). Reuses the
 * exact same decision functions scripts/ingest-requirements-deadlines.ts uses, scoped to only
 * the 4 top5_*_2026-08-23.jsonl files, printing per-record outcome+detail instead of just
...
async function main() {
  ...
  console.log("\n=== WARWICK SUPPLEMENTAL CHECK (candidate pool excludes the superseded duplicate ad3ef0a4-1502-4bca-bc2c-69c71e40e2d5) ===");
  const universitiesNoSupersededWarwick = universities.filter((u) => u.id !== "ad3ef0a4-1502-4bca-bc2c-69c71e40e2d5");
  const warReq = reqRecords.find((r) => r.research_requirement_id === "REQ-2026-08-23-WAR0001")!;
  const warDecisionReq = decideRequirementIngestion(warReq, universitiesNoSupersededWarwick, new Set(), existingTitlesByKey);
  console.log(`${warReq.research_requirement_id} -> ${warDecisionReq.outcome}${warDecisionReq.detail ? " :: " + warDecisionReq.detail : ""}`);
  if (warDecisionReq.outcome === "accepted" && warDecisionReq.row) {
    console.log(`    university_id=${warDecisionReq.row.university_id} type=${warDecisionReq.row.requirement_type} is_required=${warDecisionReq.row.is_required} confidence=${warDecisionReq.row.data_confidence}`);
  }
  for (const id of ["DL-2026-08-23-WAR0001", "DL-2026-08-23-WAR0002"]) {
    const rec = dlRecords.find((r) => r.research_deadline_id === id)!;
    const dec = decideDeadlineIngestion(rec, universitiesNoSupersededWarwick, existingDeadlineKeys);
    console.log(`${id} -> ${dec.outcome}${dec.detail ? " :: " + dec.detail : ""}`);
    if (dec.outcome === "accepted" && dec.row) {
      console.log(`    university_id=${dec.row.university_id} type=${dec.row.deadline_type} date=${dec.row.deadline_date} recurrence=${dec.row.recurrence} cycle=${dec.row.cycle_year} verification=${dec.row.verification_state}`);
    }
  }
}
