 ...
 *
 * A GENUINE STRUCTURAL DIFFERENCE FROM THE UK BATCH: every Canadian university checked this
 * pass bills tuition PER CREDIT, not as one flat annual figure — this is normal here, not a
 * gap. Deliberately NOT multiplying by an assumed credit-load to manufacture an annual figure
...
interface CaTuitionEntry {
  /** CAD/credit — [low, high] across published undergraduate programs for students starting
   * 2026/27. */
  internationalPerCredit: [number, number];
  domesticPerCreditLow?: number;
  sourceUrl: string;
  notes: string;
}
...
const CA_TUITION: Record<string, CaTuitionEntry> = {
  "University of British Columbia": {
    internationalPerCredit: [1434.7, 2222.61],
    domesticPerCreditLow: 206.69,
...
async function main(): Promise<void> {
  ...
  for (const uni of caUnis ?? []) {
    ...
    toWrite.push({ universityId: uni.id, universityName: uni.name, metricCode: "tuition_international_per_credit", valueNumeric: entry.internationalPerCredit[0] });
    if (entry.domesticPerCreditLow != null) {
      toWrite.push({ universityId: uni.id, universityName: uni.name, metricCode: "tuition_domestic_per_credit", valueNumeric: entry.domesticPerCreditLow });
    }
  }

  console.log(`${toWrite.length} metric rows to write, ${noEntry} Canadian universities with no entry in this table (left unresolved, see file header for the two genuine "too complex to resolve this pass" cases and the queued remainder).`);

  if (!apply) {
    console.log("\nDry run — nothing written. Re-run with --apply.");
    for (const w of toWrite) console.log(`  would write ${w.universityName} / ${w.metricCode} = $${w.valueNumeric}/credit`);
    return;
  }
