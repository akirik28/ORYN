-- ═══════════════════════════════════════════════════════════════════════════
-- PROXOLA — 4 Eylül, ÜNİVERSİTE VERİSİ paketi
-- ═══════════════════════════════════════════════════════════════════════════
--
-- ⚠ ÖNCE 09, SONRA 11, EN SON BU.
--   Bu paket 0119'a dayanıyor ve 0119 da mevcut şemaya. Sıra bozulursa
--   dosya kendini durdurur, hiçbir şey uygulanmaz.
--
-- ⚠ BU PAKET VERİ YAZIYOR — diğer ikisi sadece şema değiştiriyordu.
--   Dokuz üniversitenin eksik alanları dolduruluyor, ve **bir tanesi
--   düzeltme**: TU Münih'in uluslararası öğrenci ücreti şu an "0" yazıyor
--   ve bu yanlış. Aşağıda ayrıca anlatıyorum.
--
-- İÇİNDEKİLER
--   1. Migration 0119 — kabul oranı boşsa "araştırılmadı" mı, "böyle bir
--      sayı yok" mu, onu ayırt eden sütun
--   2. Dokuz üniversitenin verisi (düzeltme + eklemeler)
--   3. Doğrulama — neyin gerçekten değiştiğini tek tek rapor eder
--
-- Yeni sekme, Cmd+A, Delete, tamamını yapıştır, Run. `commit;` ile biter.
--
-- Canlı veritabanına hiçbir yazma yapılmadı; bu dosya senin çalıştırman için.
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  if not exists (select 1 from information_schema.tables
    ...
    raise exception E'\n\n09 numarali paket henuz uygulanmamis. Once 09, sonra 11, en son bu.\nHicbir sey uygulanmadi.\n';
  end if;
end $$;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 3. Doğrulama — neyin GERÇEKTEN değiştiğini raporlar
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Bu blok sadece bakmaz, sayar. Özellikle TU Münih düzeltmesi için:
-- düzeltme bir koruma taşıyor (sadece mevcut değer 0 ise çalışır), yani
-- değer başka bir şeyse **sessizce hiçbir şey yapmaz.** O sessizliği
-- görmen için ayrıca raporluyorum.
...
begin
  if not exists (select 1 from information_schema.columns
    where table_schema='public' and table_name='university_statistics'
      and column_name='admission_rate_basis') then
    eksik := eksik || '0119: university_statistics.admission_rate_basis';
  end if;
  ...
  select value_numeric, source_url into tum_deger, tum_kaynak
  from public.university_profile_metrics
  where university_id = '52409036-32ff-47ff-9815-c96a4bc89125'
    and metric_code = 'tuition_international_annual';

  raise notice '─────────────────────────────────────────────';
  raise notice '0119 YERINDE — admission_rate_basis';
  raise notice '';
  if tum_deger is null then
    raise notice 'TU MUNIH: satir bulunamadi — DUZELTME UYGULANMADI, bana soyle';
  elsif tum_deger = 4000 then
    raise notice 'TU MUNIH: DUZELTILDI — 0 idi, simdi 4000 EUR/yil (alt sinir)';
    raise notice '          kaynak: %', tum_kaynak;
  elsif tum_deger = 0 then
    raise notice 'TU MUNIH: HALA 0 — duzeltme calismadi, bana soyle';
  else
    raise notice 'TU MUNIH: deger % — beklenen 0 veya 4000 degil, ELLE BAK', tum_deger;
  end if;
  raise notice '─────────────────────────────────────────────';
end $$;

-- Dokuz üniversitenin son durumu, tek tabloda.
select
  u.name as universite,
  (select count(*) from public.university_requirements r where r.university_id = u.id) as gereksinim,
  (select count(*) from public.university_programs p where p.university_id = u.id)     as program,
  (select count(*) from public.university_sources s where s.university_id = u.id)      as kaynak,
  (select count(*) from public.university_statistics t where t.university_id = u.id)   as istatistik
from public.universities u
where u.id in (
  select distinct university_id from public.university_sources
  where retrieved_at::date = '2026-09-04'
)
order by u.name;
